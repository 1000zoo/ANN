...
"""
target_size = (128, 128)  /  batch_size = 20  /  class_mode = 'binary'

## model & classifier
model => VGG16(weights = 'imagenet')
GlobalAveragePooling2D => Dense(512) => BatchNormalization =>
Activation(Relu) => Dense(128) => Dense(1)

## 2 step fine tuning
100 epochs + 50 fine tuning epochs (tune 5 blocks)
...
plot_loss(history)
plt.savefig('Q1-loss.png')
plt.clf()
plot_acc(history)
plt.savefig('Q1-accuracy.png')
plt.clf()
...
model.save("chest_x_ray_2step_01.h5")
